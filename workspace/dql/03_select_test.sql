...
select eno, ename, salary, nvl(commission,0)+300 인상급여 from employee;

-- 2. 연간 총 수입이 많은 순으로 출력 (O)
select ename, salary, salary*12 from employee order by salary desc;
...
-- 5. 급여가 2000에서 3000 사이에 포함되지 않는 사람 출력 (O)
select ename, dno, salary from employee where not salary between 2000 and 3000;
...
select ename, d.dname from employee e, department d where e.manager is null and d.dno=e.dno;

-- 11. 커미션을 받을 수 있는 사람을 급여 및 커미션 기준으로 내림차순 (X)(order by desc 둘 다 줘야함)
select ename, salary, commission from employee where commission is not null order by salary, commission desc;
...
select ename, job, dname, salary from employee e, department d 
